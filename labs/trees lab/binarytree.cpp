...
template <typename T>
void BinaryTree<T>::printPaths(const Node* subRoot, T path[], int pathLen) const
{
  // Your code here
  ...
  if (subRoot->left == NULL && subRoot->right == NULL) {
    ...
    printPaths(subRoot->left, path, pathLen);
    printPaths(subRoot->right, path, pathLen);
  }
}
...
int BinaryTree<T>::sumDistances() const
{
  // Your code here
  int sum = 0;
  sumDistances(root, 0, sum);
  return sum;
}

template <typename T>
void BinaryTree<T>::sumDistances(const Node* subRoot, int pathLen, int& sum) const {
  // Your code here
  if (!subRoot) return;
  sum += pathLen;
  sumDistances(subRoot->left, pathLen+1, sum);
  sumDistances(subRoot->right, pathLen+1, sum);
}

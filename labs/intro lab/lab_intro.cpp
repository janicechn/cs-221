...
for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if (d > 200) {
        pixel->l = 0; //luminance is 0 at distance over 200 pixels away
      } else {
        //decrease by 0.5% by distance
        pixel->l *= (1-0.005*d);
        //long way: pixel->l = pixel->l*(1-0.005*d);
      }
    }
  }
...
PNG ubcify(PNG image) {
  //hue values for UBC Yellow and Blue
  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double& hue = pixel->h;
      if(abs(hue - YELLOW) < abs(hue - BLUE)) {
        hue = YELLOW;
      } else {
        hue = BLUE;
      }
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
      ...
      double& lum = pixelFirst->l;
      if(pixelSecond->l == 1) { //checks l from second image being 1 (100%)
        lum += 0.2; //changes l from first image
      }
    }
  }
  return firstImage;

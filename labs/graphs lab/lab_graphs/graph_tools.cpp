...
// Label vertices and edges unexplored
void GraphTools::unexploreVE(Graph& graph) {
...
int GraphTools::findMinWeight(Graph& graph)
{
    ...
    unexploreVE(graph);
    Edge e = BFS(graph, graph.getStartingVertex());
...
int GraphTools::findShortestPath(Graph& graph, Vertex start, Vertex end)
{
    ...
    int count = 0;
    while (end != start) {
        graph.setEdgeLabel(start, parent[start], "MINPATH");
        start = parent[start];
        count++;
    }
    return count;

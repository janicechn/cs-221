...
void AVLTree<K, V>::rebalance(Node*& subtree)
{
    ...
    if ((subtree->left != NULL) && (height(subtree->left) - 2 == height(subtree->right))) {
        if (height(subtree->left->left) - 1 == height(subtree->left->right)) {
            rotateRight(subtree);
        } else {
            rotateLeftRight(subtree);
        }
    } else if ((subtree->right != NULL) && (height(subtree->left) + 2 == height(subtree->right))) {
        if (height(subtree->right->right) - 1 == height(subtree->right->left)) {
            rotateLeft(subtree);
        } else {
